...
$pdo->prepare("UPDATE fahrzeuge SET status = 'available', vermietet_bis = NULL WHERE status = 'unavailable' AND vermietet_bis IS NOT NULL AND STR_TO_DATE(vermietet_bis, '%d.%m.%Y %H:%i') <= ?")
    ->execute([$now]);
// Fahrzeuge mit Kategorie "nutzfahrzeuge"
$stmt = $pdo->prepare("SELECT * FROM fahrzeuge WHERE kategorie = ?");
$stmt->execute(['nutzfahrzeuge']);
...
<head>
  ...
  <title>Nutzfahrzeuge – J+T Rental</title>
...
<header class="navbar">
  ...
  <nav>
    <ul class="nav-links">
      ...
      <li><a href="nutzfahrzeuge.php" class="active">Nutzfahrzeuge</a></li>
    ...
    if (json.success) {
      ...
      bestaetigung.innerHTML = `
       <strong>${json.message}</strong><br><br>
        ...
        <a href="nutzfahrzeuge.php" style="color:#fff; text-decoration:underline;">Zurück zur Übersicht</a>
      `;
    } else {

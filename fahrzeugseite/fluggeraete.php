...
$pdo->prepare("UPDATE fahrzeuge SET status = 'available', vermietet_bis = NULL WHERE status = 'unavailable' AND vermietet_bis IS NOT NULL AND STR_TO_DATE(vermietet_bis, '%d.%m.%Y %H:%i') <= ?")
    ->execute([$now]);
// Fahrzeuge mit Kategorie "fluggeraete"
$stmt = $pdo->prepare("SELECT * FROM fahrzeuge WHERE kategorie = ?");
$stmt->execute(['fluggeraete']);
...
<head>
  ...
  <title>Fluggeräte – J+T Rental</title>
...
<header class="navbar">
  ...
  <nav>
    <ul class="nav-links">
      ...
      <li><a href="fluggeraete.php" class="active">Fluggeräte</a></li>
    ...
    if (json.success) {
      ...
      bestaetigung.innerHTML = `
       <strong>${json.message}</strong><br><br>
        ...
        <a href="fluggeraete.php" style="color:#fff; text-decoration:underline;">Zurück zur Übersicht</a>
      `;
    } else {
